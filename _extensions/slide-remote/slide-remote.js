(() => {
  // packages/deck-plugin/src/sanitize.ts
  ...
  // packages/deck-plugin/src/extract.ts
  var MAX_NOTES_BYTES = 64 * 1024;
  ...
  // packages/deck-plugin/src/client.ts
  var RECONNECT_BACKOFF_MS = [500, 1000, 2000, 4000, 8000, 15000];
  var MAX_RECONNECT_ATTEMPTS = 60;
  function applyRemoteCommand(reveal, cmd, args) {
    switch (cmd) {
      case "next":
        reveal.next();
        break;
  ...
  class Client {
    ...
    async start() {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(`mint room: ${res.status}`);
        this.room = await res.json();
      } catch (e) {
        this.handlers.onError(`could not mint room: ${e.message}`);
    ...
    attachRevealHooks() {
      ...
      for (const ev of events)
        this.reveal.on(ev, () => this.pumpStateSoon());
    }
    pumpStateSoon() {
      if (this.flushTimer)
        return;
  ...
  function readConfig() {
    return {
      ...
      disableOnParams: meta("slide-remote-disable-on-params").split(",").map((s) => s.trim()).filter(Boolean)
    };
  }
  function shouldDisable(cfg) {
    if (typeof window === "undefined")
      return true;
    if (!cfg.workerUrl)
      return true;
    if (meta("slide-remote-enabled") === "false")
      return true;
...
`;
  ...
  // packages/deck-plugin/src/qr.ts
  function qrSvg(text, sizePx = 256) {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${sizePx} ${sizePx}" width="${sizePx}" height="${sizePx}">` + `<rect width="${sizePx}" height="${sizePx}" fill="#ffffff"/>` + `<path d="${path}" fill="#000000"/>` + `</svg>`;
  }

  // packages/deck-plugin/src/overlay.ts
  ...
  class Overlay {
    ...
    constructor(handlers) {
      ...
      const meta2 = el("div", "sr-overlay__meta");
      meta2.append(row("Room", this.codeEl), row("Status", this.statusEl), row("Phones", this.peerEl));
      ...
      panel.append(closeBtn, el("h2", "sr-overlay__title", "Pair your phone"), this.qrHost, meta2, this.linkEl, el("p", "sr-overlay__hint", "Scan with your iPhone camera. Press Esc to dismiss."));
      ...
      document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && this.root.isConnected)
          handlers.onClose();
      });
    }
    open(joinUrl, roomId) {
  ...
  // packages/deck-plugin/src/index.ts
  var STATUS_TEXT = {
  ...
  class SlideRemoteController {
    ...
    activate() {
      if (this.client) {
        ...
        if (room)
          this.overlay?.open(room.joinUrl, room.roomId);
        return;
      }
