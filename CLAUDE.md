...
- **The committed deck-plugin bundle in `_extensions/slide-remote/slide-remote.js` is the source of truth for `quarto add` consumers** — the extension is fetched by git tag, not from `main`. Any source change in `packages/deck-plugin/src/` MUST be followed by `bun run build:plugin:min` and a commit of the regenerated bundle. The `bundle-fresh` CI job enforces this. (`scripts/check-bundle-fresh.ts`)
...
- **30 KB gzip budget on the deck-plugin bundle.** Single bundle (`slide-remote.js`); the QR library is inlined. **If you add a sibling file to the plugin dir (a second script, a font, an image), it MUST be listed under `_extension.yml`'s `script:`/`stylesheet:` — Quarto only copies what's declared, and silently drops files referenced only at runtime.** This is what bit v0.5.2's lazy-loaded QR chunk; the inline rewrite landed in v0.5.3.
...
- **Wrangler migration tags are append-only.** Each `[[migrations]]` block in `packages/worker/wrangler.toml` becomes permanent in your deployed prod's history once shipped — never reorder, rename, or delete a tag post-deploy. To add a new DO class, add a fresh tag at the bottom. SQLite-backed classes (`new_sqlite_classes`) cannot migrate back to KV-style storage; the choice is one-way. Pin the compat date carefully when bumping wrangler.
